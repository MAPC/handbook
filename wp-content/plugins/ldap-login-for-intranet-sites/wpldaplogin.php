			...
			<h3>Extra profile information</h3>
				...
				<table class="form-table">
					...
					<tr>
                        ...
                        <td>
                            <b><?php echo esc_attr( get_the_author_meta( 'mo_ldap_user_phone', $user->ID ) ); ?></b></td>
                        </td>
                    </tr>
						<?php
		...
		function ldap_login($user, $username, $password){
			...
			if($auth_response->statusMessage == 'SUCCESS'){
			  ...
			  if( username_exists( $username) || email_exists($username)) {
				  ...
				  update_user_meta($user->ID, 'mo_ldap_user_phone', $profile_attributes['phone']);
					   ...
					   if(!get_option('mo_ldap_local_register_user')) {
						  ...
						  if($fallback_login_enabled)
						   ...
						   $fname = $profile_attributes['fname'];
							 ...
							 $phone = $profile_attributes['phone'];
							...
							if( !is_wp_error($user_id) ) {
								$user = get_user_by("login",$username);
								  ...
								  //Store distinguishedName in User Meta
								   ...
								   //Store phone number in User Meta
				  				   update_user_meta($user->ID, 'mo_ldap_user_phone', $phone);
